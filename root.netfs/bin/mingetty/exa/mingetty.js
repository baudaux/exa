...
var TOTAL_STACK = 32768;
if (Module['TOTAL_STACK']) assert(TOTAL_STACK === Module['TOTAL_STACK'], 'the stack size can no longer be determined at runtime')

var INITIAL_MEMORY = Module['INITIAL_MEMORY'] || 65536;legacyModuleProp('INITIAL_MEMORY', 'INITIAL_MEMORY');
...
assert(INITIAL_MEMORY == 65536, 'Detected runtime INITIAL_MEMORY setting.  Use -sIMPORTED_MEMORY to define wasmMemory dynamically');
...
function createWasm() {
  ...
  function receiveInstance(instance, module) {
    ...
    //assert(wasmMemory.buffer.byteLength === 65536);
...
function environ_get(env,buf) { if (Module['env']) { Module.HEAPU8.set(Module['env'].buf, buf); let count = 0; for (let i = 0; i < Module['env'].size;) { if (count >= Module['env'].count) break; Module.HEAPU8[env+4*count] = (buf+i) & 0xff; Module.HEAPU8[env+4*count+1] = ((buf+i) >> 8) & 0xff; Module.HEAPU8[env+4*count+2] = ((buf+i) >> 16) & 0xff; Module.HEAPU8[env+4*count+3] = ((buf+i) >> 24) & 0xff; let j = 0; for (; Module.HEAPU8[buf+i+j]; j++) ; i += j+1; count++; } } }
  ...
  var TTY = {ttys:[],init:function () {
        ...
        FS.registerDevice(dev, TTY.stream_ops);
          ...
          return ret;
        }},default_tty_ops:{get_char:function(tty, callback, blocking) {
          if (!tty.input.length) {
            var result = null;
            if (ENVIRONMENT_IS_NODE) {
          ...
          switch(cmd) {
            ...
            case 21508: {
              ...
              msg.termios = {
                ...
                c_iflag: HEAPU32[((arg)>>2)],
                c_oflag: HEAPU32[(((arg)+(4))>>2)],
                c_cflag: HEAPU32[(((arg)+(8))>>2)],
                c_lflag: HEAPU32[(((arg)+(12))>>2)],
              };
              ...
              if (e.type == 2) {    // IOCTL
                ...
                switch(e.op) {
                  ...
                  case 0:       // TCGETS
                    ...
                    HEAP32[((arg)>>2)] = e.termios.c_iflag;
                    HEAP32[(((arg)+(4))>>2)] = e.termios.c_oflag;
                    HEAP32[(((arg)+(8))>>2)] = e.termios.c_cflag;
                    HEAP32[(((arg)+(12))>>2)] = e.termios.c_lflag;
                    HEAP32[(((arg)+(52))>>2)] = e.termios.c_ispeed;
                    HEAP32[(((arg)+(56))>>2)] = e.termios.c_ospeed;
                  ...
                  case 2:       // TIOCGWINSZ
                    ...
                    HEAPU16[((arg)>>1)] = e.winsize.ws_row;
                    HEAPU16[(((arg)+(2))>>1)] = e.winsize.ws_col;
  ...
  var SOCKFS = {mount:function(mount) {
  	...
  	return Module['fd_table'][fd];
        ...
        return 'socket[' + (SOCKFS.nextname.current++) + ']';
      },unix_dgram_sock_ops:{bind:function(sock, addr, port) {
  	      ...
  	      HEAP16[((addr)>>1)] = 1; // family: AF_UNIX (1)
  	      stringToUTF8(msg.from, addr + 2, 108);
  	      HEAP32[((addrlen)>>2)] = 110;
  ...
  function ___syscall_dup2(fd, new_fd) {
  try {
  ...
  function ___syscall_execve(pathname, argv, envp) {
  try {
  ...
  function ___syscall_getsid(req_pid) {
  try {
  ...
  function ___syscall_read(fd, buf, count) {
  try {
  ...
  function ___syscall_readv(fd, iov, iovcnt) {
  try {
  ...
  function ___syscall_writev(fd, iov, iovcnt) {
  try {
  	...
  	let ret = Asyncify.handleSleep(function (wakeUp) {
  		...
  		for (var i = 0; i < iovcnt; i++) {
  		    let ptr = HEAPU32[((iov2)>>2)];
  		    let l = HEAPU32[(((iov2)+(4))>>2)];
...
var asmLibraryArg = {
  ...
  "__syscall_getsid": ___syscall_getsid,
  "__syscall_ioctl": ___syscall_ioctl,
  "__syscall_openat": ___syscall_openat,
  "__syscall_read": ___syscall_read,
  "__syscall_readv": ___syscall_readv,
  "__syscall_sendto": ___syscall_sendto,
  "__syscall_socket": ___syscall_socket,
  "__syscall_write": ___syscall_write,
  "__syscall_writev": ___syscall_writev,
...
var ___start_em_js = Module['___start_em_js'] = 5468;
var ___stop_em_js = Module['___stop_em_js'] = 6086;
